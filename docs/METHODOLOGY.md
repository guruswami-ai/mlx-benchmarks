...
**TP3 and TP5 fail on all tested models.** Only TP2 and TP4 produce results. TP requires attention heads to divide evenly by the node count. Four of five models fail TP5 on query head maths alone (128/5, 32/5, 64/5 are not integers). Qwen 32B is the exception: 40 query heads divide cleanly by 5, but its 8 KV heads do not (8/5=1.6), and the framework refuses to initialise. Even if it loaded, our TB5 RDMA mesh was not stable at 5 nodes. See [Distributed Inference](DISTRIBUTED_INFERENCE.md) for the full head divisibility table.
